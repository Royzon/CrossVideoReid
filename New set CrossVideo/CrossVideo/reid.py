...
class CNN_deepsort:
    ...
    def compute_feature(self,im):
        ...
        return feature
class CNN_REID:
    def __init__(self,lib_dth=0.01,passage_dth=0.02,reid_dth=0.03,forget=10,kalman=False):
        ''' arg1:库添加的阈值 arg2:切换状态的阈值 arg3: 分辨不同人的阈值 arg4:状态变为隐藏的帧数 arg5:kalman开启的阀门'''
        self.frame=0
        self.color_old=0
        self.flag_frame=[]
        self.flag_line=[]
...
#*      检测函数集   
#*                  
    def check_create_id(self,bboxs,im):
        ...
        for feature in list_feature:
            ...
            for k,v in self.save.items():
                if k!='000' and v['passage']==True and v['hold']==False:
                    ...
                    dis = self.compute_cos(v['center'],feature)
                    list_dis.append(dis)
                    list_overlap_name.append(k)
            list_names.append(list_overlap_name[list_dis.index(min(list_dis))])
        return list_names

    def check_overlap_id(self,self_id):
        over_id=' '
        ids = self.save[self_id]['overlap_id']
        for id_ in ids:
            if self.save[id_]['hold']==False:
                ...
                self.save[self_id]['overlap_id'].remove(id_)
        if len(self.save[self_id]['overlap_id'])<2:
   ...
   #检查bbox是否有交叉
    ...
    def check_lib_add(self,featureVector,min_distance_id,bbox):
        for k,v in list(self.save.items()):
            ...
            if  k==min_distance_id:
                n=v['center']
                lib = v['lib']
                self.save[k]['bbox']=bbox
                #跟新kalman 轨迹预测
                if self.kalman_switch:
                    self.kalman_bbox_update(k,bbox)
                for lib_ in lib:
    ...
    def None_mode(self,notebook,im):
        ...
        for bbox,c in notebook.items():
            ...
            for k,v in list(self.save.items()):
                if k!='000' and v['passage']==True and v['hold']==False and v['overlap']==False:
                    n=v['center']
                    if self.kalman_switch:
                        kalman_xy=v['kalman']
        ...
        if CNN_bbox!=[] and list_distance!=[]:
                ...
                if len(list_distance)>0:
                    ...
                    num_CNN=0   
                    #print(list_Hungary,np_distance)
    ...
    def Cross_mode(self,notebook,im):     
        ...
        if CNN_bbox!=[] and list_distance!=[]:
                ...
                for bbox,c in zip(list_bbox_loss,confidences_loss):
                    notebook_to_None[bbox]=c
        self.None_mode(notebook_to_None,im)
    #OverLap 模式
        ...
        for root_bbox,v in notebook.items():
            ...
            if v[0]=='create':
                ...
                if len(create_bboxs)>=2:
                    list_ids = self.check_create_id(create_bboxs,im)
                    if len(list_ids)>=len(create_bboxs):
                        overlap_id = self.add_lib_overlab(featureVector,root_bbox,self.frame,list_ids)
                        #print(list_ids)
            ...
            if v[0]=='inherit':
                c,self_id=v[1],v[2]
                over_id = self.check_overlap_id(self_id)
                if over_id!=' ':
                    color_ = self.check_lib_add(featureVector,self_id,root_bbox)
                    self.save[self_id]['frame']=self.frame
                    self.save[self_id]['overlap']=True
                    self.save[self_id]['center']=featureVector
            ...
            if v[0]=='combine_split':
                ...
                feture_source=self.save[self_id]['center']
                #去除与self_id相同的bbox 
                for bbox_ in bboxs:
                    (x, y, w, h) = bbox_
                    (x1, y1, x2, y2)=abs(int(x-w/2)), abs(int(y-h/2)), int(x+w/2), int(y+h/2)
                ...
                for bbox_,feature1 in zip(bboxs,list_features):
                    ...
                    for id_ in self.save[self_id]['overlap_id']:
                        list_=[]
                    ...
                    if min(min_diantance)>self.reid_dth:
                        ...
                        if len(min_distance)>0:
                            ...
                            #添加id到overlap_id 里
                            if id_combine not in self.save[self_id]['overlap_id']:
                                if len(self.save[id_combine]['overlap_id'])>0:
                                    self.save[self_id]['overlap_id']+=self.save[id_combine]['overlap_id']
                                    self.save[self_id]['overlap_id']=list(set(self.save[self_id]['overlap_id']))
                                    self.dele_lib_id(id_combine)
                                else:
                                    self.save[self_id]['overlap_id'].append(id_combine)
                            #取出ID合集
                            over_id = self.check_overlap_id(self_id)
                            if over_id!=' ':
                                color_ = self.check_lib_add(featureVector,self_id,root_bbox)
    ...
    #overlap bbox查找函数
    def check_bboxs_overlap(self,bboxs,confidences,im):
        ...
        if len(notebook_.keys())>0:
            ...
            for bbox_new,bbox_father in notebook_.items():
                ...
                for k,v in self.save.items():
                    if v['overlap']==True:
                        iou1,iou2 = self.compute_IOU_myself(v['bbox'],bbox_new)
        ...
        if len(myself_bboxs)!=0:
            for k,v in self.save.items():
                if v['overlap']==True and v['hold']==False:
                    list_bboxs,list_ious,list_c=[],[],[]
                    for bbox_,c_ in zip(myself_bboxs,myself_confidences):
                        iou1,iou2 = self.compute_IOU_myself(v['bbox'],bbox_)
                        if iou1>0.2 or iou2>0.2:
    ...
    #bbox选则函数
    def check_bboxs_select(self,bboxs,confidences,im):
        #3个状态存储
        cross_notebook,none_notebook,overlap_notebook={},{},{}
        #1.从bboxs中找到overlap继承的bbox,并剔出
        #2.从bboxs中找到合并的，并剔出
        overlap_notebook,bbox_none,out_bboxs,confidences_ = self.check_bboxs_overlap(bboxs,confidences,im)
        if len(bbox_none)>0:
    ...
    #*主函数 
    #*输入：bbox的列表，confidence的列表，原图片
    #*输出：bbox的列表， id的列表 ，confidence的列表 ，bbox颜色列表（供选用），帧（供选用）
    #*
    def cross_video(self,bbox_list,confidences,im):
